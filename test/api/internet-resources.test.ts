...
// Several targets (e.g. StackOverflow, GitHub) front their traffic with bot
// protection that blocks requests without a browser-like User-Agent header.
...
void describe('Required Internet resource', () => {
  void describe('PasteBin paste for "Leaked Unsafe Product" challenge available', () => {
    void it('for default configuration (https://pastebin.com/90dUgd7s)', async () => {
      const res = await fetch('https://pastebin.com/90dUgd7s', { headers: browserHeaders })
    ...
    void it('for 7MS configuration (https://pastebin.com/8SMbWPxc)', async () => {
      const res = await fetch('https://pastebin.com/8SMbWPxc', { headers: browserHeaders })
      const body = await res.text()
      assert.equal(res.status, 200)
    ...
    void it('for BodgeIt Store configuration (https://pastebin.com/G47LrDr0)', async () => {
      const res = await fetch('https://pastebin.com/G47LrDr0', { headers: browserHeaders })
      const body = await res.text()
      assert.equal(res.status, 200)
    ...
    void it('for Mozilla configuration (https://pastebin.com/t8jqE1y7)', async () => {
      const res = await fetch('https://pastebin.com/t8jqE1y7', { headers: browserHeaders })
      const body = await res.text()
      assert.equal(res.status, 200)
    ...
    void it('for All Day DevOps configuration (https://pastebin.com/RXrihEMS)', async () => {
      const res = await fetch('https://pastebin.com/RXrihEMS', { headers: browserHeaders })
      const body = await res.text()
      assert.equal(res.status, 200)
  ...
  void it('Comment on "Top 10 Fruits you probably dont know" blog post with PasteBin paste URL spoiler available', async () => {
    const res = await fetch('https://listverse.disqus.com/top_20_fruits_you_probably_don039t_know/latest.rss', { headers: browserHeaders })
    const body = await res.text()
    assert.equal(res.status, 200)
  ...
  void it('PasteBin paste (https://pastebin.com/4U1V1UjU) for "Leaked Access Logs" challenge available', async () => {
    const res = await fetch('https://pastebin.com/4U1V1UjU', { headers: browserHeaders })
    const body = await res.text()
    assert.equal(res.status, 200)
  ...
  void it('StackOverflow question "Less verbose access logs using expressjs/morgan" with log snippet and PasteBin paste URL spoiler available', async (t) => {
    const res = await fetch('https://stackoverflow.com/questions/57061271/less-verbose-access-logs-using-expressjs-morgan', { headers: browserHeaders })
    if (res.status === 403 && res.headers.get('cf-mitigated') === 'challenge') {
      t.skip('StackOverflow served a Cloudflare bot challenge instead of the page; cannot verify from a non-browser HTTP client')
      return
    }
    const body = await res.text()
    assert.equal(res.status, 200)
